...
exports.parseComponentSpecifier = parseComponentSpecifier;
exports.resolveRegistry = resolveRegistry;
exports.fetchComponent = fetchComponent;
exports.collectComponents = collectComponents;
exports.downloadRegistryFile = downloadRegistryFile;
const node_fetch_1 = __importDefault(require("node-fetch"));
const config_js_1 = require("./config.js");
function parseComponentSpecifier(input, config) {
    ...
    if (trimmed.startsWith('@')) {
        ...
        if (parts.length !== 2 || !parts[0] || !parts[1]) {
            throw new Error(`Invalid component specifier "${input}". Use "button" or "@scope/component".`);
        }
        const scope = parts[0];
...
function resolveRegistry(scope, config) {
    const registryUrl = config.registries[scope];
    if (!registryUrl) {
        throw new Error(`Unknown registry scope "${scope}". Add it to "registries" in watermelon.json.`);
    }
    return getRegistryTemplate(registryUrl);
}
async function fetchComponent(specifier) {
    const manifestUrl = specifier.manifestUrl;
    let response;
    try {
        response = await (0, node_fetch_1.default)(manifestUrl);
    }
    catch (error) {
        throw new Error(`Network error while fetching "${specifier.raw}" from ${manifestUrl}: ${getErrorMessage(error)}`);
    }
    if (response.status === 404) {
        throw new Error(`Component "${specifier.raw}" was not found at ${manifestUrl}.`);
    }
    if (!response.ok) {
        throw new Error(`Failed to fetch "${specifier.raw}" from ${manifestUrl}: ${response.status} ${response.statusText}`);
    }
    let data;
    try {
        ...
        throw new Error(`Invalid JSON received for "${specifier.raw}" from ${manifestUrl}: ${getErrorMessage(error)}`);
    }
    return validateManifest(data, specifier.raw);
}
async function collectComponents(inputs, config) {
    const resolved = new Map();
    async function visit(input, parentScope) {
...
async function downloadRegistryFile(fileBaseUrl, file) {
    const sourceUrl = file.url ?? buildFileUrl(fileBaseUrl, file.path);
    let response;
    try {
        response = await (0, node_fetch_1.default)(sourceUrl);
    }
    catch (error) {
        throw new Error(`Network error while downloading "${file.path}" from ${sourceUrl}: ${getErrorMessage(error)}`);
    }
    if (!response.ok) {
        throw new Error(`Failed to download "${file.path}" from ${sourceUrl}: ${response.status} ${response.statusText}`);
    }
    return response.text();
}
function buildManifestUrl(registrySource, componentName) {
...
function buildFileUrl(fileBaseUrl, filePath) {
...
function buildFileBaseUrl(registrySource) {
    const template = typeof registrySource === 'string'
        ? registrySource
        : getRegistryTemplate(registrySource);
...
function getRegistryTemplate(registry) {
    return (0, config_js_1.isRegistryDirectoryEntry)(registry) ? registry.url : registry;
}
function validateManifest(data, name) {
